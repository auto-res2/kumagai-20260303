...
Aggregate metrics. For each method, we compute final-answer accuracy as \texttt{correct\_count} / \texttt{total\_samples}. We measure average tokens per sample from API token usage metadata and aggregate estimated cost in USD from a simple provider-specific estimator. The primary metric in the logged summaries is \texttt{reliability\_per\_token} defined as (accuracy * 1000) / \texttt{avg\_tokens\_per\_sample}, so higher values indicate more correct answers per generated token. For C3oT, we additionally log \texttt{commitment\_consistency} (fraction of examples with valid commitment JSON), \texttt{invariant\_satisfaction} (fraction passing the heuristic invariant or evidence check on the solve pass), and \texttt{repair\_rate} (fraction of examples that triggered a repair call).
...
Batching and runtime controls. Inference is executed with \texttt{batch\_size} set to 4 (parallel workers). The code supports a \texttt{sanity\_check} mode that would reduce the dataset to 10 samples, but the reported results are from the full 200-sample runs.

Evaluation metrics and scoring. The evaluation code computes: \texttt{final\_answer\_accuracy}, \texttt{tokens\_used}, \texttt{avg\_tokens\_per\_sample}, \texttt{cost\_usd}, and \texttt{reliability\_per\_token} for both runs. For C3oT it also computes \texttt{commitment\_consistency}, \texttt{invariant\_satisfaction}, and \texttt{repair\_rate}. Correctness for both methods is determined by comparing the extracted \texttt{final\_answer} to the \texttt{ground\_truth} with an absolute tolerance of $1 \times 10^{-6}$.
...
Primary metric: reliability per token (higher is better). Using the logged definition \texttt{reliability\_per\_token} = (accuracy * 1000) / \texttt{avg\_tokens\_per\_sample}, the baseline achieves 2.1676, while C3oT achieves 0.6694. The gap (proposed minus baseline) is -1.4983, indicating that C3oT is substantially worse on the primary efficiency-oriented metric.

C3oT contract metrics (higher is better). C3oT’s \texttt{commitment\_consistency} is 0.98, meaning the commitment JSON parsed and passed schema validation for 98% of examples. The \texttt{invariant\_satisfaction} rate is 0.565, meaning that 56.5% of examples passed the heuristic invariant or evidence validation on the solve pass. The \texttt{repair\_rate} is 0.415, indicating that 41.5% of examples triggered the repair prompt due to initial invariant validation failure.

Figures. The following figures were produced by the evaluation pipeline and are included here exactly once each.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/comparative-1-baseline-cot-gsm8k_metrics.pdf}
\caption{Per-run metrics over time for the baseline bounded CoT run on GSM8K; higher values indicate better performance for final-answer accuracy and reliability per token, while lower values indicate better performance for token and cost metrics.}
\label{fig:baseline-metrics}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/proposed-c3ot-gsm8k_metrics.pdf}
\caption{Per-run metrics over time for the proposed C3oT run on GSM8K; higher values indicate better performance for final-answer accuracy and reliability per token, while lower values indicate better performance for token and cost metrics.}
\label{fig:c3ot-metrics}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/comparison_avg_tokens_per_sample.pdf}
\caption{Comparison of average tokens per sample across runs; lower values indicate better efficiency.}
\label{fig:comparison-tokens}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/comparison_cost_usd.pdf}
\caption{Comparison of estimated USD cost across runs; lower values indicate better efficiency.}
\label{fig:comparison-cost}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/comparison_final_answer_accuracy.pdf}
\caption{Comparison of final-answer accuracy across runs; higher values indicate better performance.}
\label{fig:comparison-accuracy}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/comparison_heatmap.pdf}
\caption{Heatmap comparison of normalized metrics across runs; higher normalized values indicate better performance for the underlying metric as logged.}
\label{fig:comparison-heatmap}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/comparison_reliability_per_token.pdf}
\caption{Comparison of reliability per token across runs; higher values indicate better performance.}
\label{fig:comparison-rpt}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/c3ot_methodology.pdf}
\caption{C3oT methodology diagram; this figure is descriptive and does not encode a performance metric.}
\label{fig:c3ot-methodology}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/system_architecture.pdf}
\caption{System architecture diagram for the experimental pipeline; this figure is descriptive and does not encode a performance metric.}
\label{fig:system-architecture}
\end{figure}

Interpretation and limitations. The comparison plots show that C3oT consumes substantially more tokens than the baseline (\cref{fig:comparison-tokens) and achieves lower final-answer accuracy (\cref{fig:comparison-accuracy}, which together yield a large drop in reliability per token (\cref{fig:comparison-rpt}. At the same time, the C3oT run’s high commitment consistency indicates that the structured commitment format is robustly elicitable, while the invariant satisfaction rate and repair rate indicate that the checker surfaces many failures that would not be observable in a standard CoT baseline. A key limitation is that invariant checking is heuristic rather than true expression evaluation, which may lead to both false failures and false passes relative to the intended mechanically checkable invariants. Another limitation is that the aggregate metrics do not include a post-repair invariant satisfaction rate, so we cannot quantify how often the repair step resolves contract failures beyond noting how frequently it is invoked.
...
On a deterministic 200-problem subset of the GSM8K test split, the recorded results do not support the hypothesis that C3oT improves reliability per token relative to a bounded standard CoT baseline. The baseline attained higher accuracy (0.935 versus 0.875) while using substantially fewer tokens per sample (431.345 versus 1307.23), resulting in a much higher \texttt{reliability\_per\_token} (2.1676 versus 0.6694).

Despite underperforming on end-task accuracy and efficiency in this implementation, C3oT demonstrated two practically relevant properties. First, it produced structurally valid commitments at high rates (\texttt{commitment\_consistency} 0.98), suggesting that the required structured artifact is feasible to elicit without fine-tuning. Second, it provided measurable auditing signals, with only 0.565 \texttt{invariant\_satisfaction} under the current heuristic checker and a \texttt{repair\_rate} of 0.415, indicating that many responses violate the declared evidence or contract requirements.
